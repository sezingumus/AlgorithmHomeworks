...
"""
import math
#Each city object has x , y coordinates and neighbors
class City(object):     
...
#We import cities from entered input file and find and bound neighbors to each cities.      
...
# This method calculates distance between two cities .
def calculateDistance(x1,x2,y1,y2):
        ...
        d = round(d)
        return d
#This function bounds neighbors to cities.     
def boundNeighborsToACity(list_of_cities):
 ...
 #This function finds minimum path between cities and returns path and min cost of the tour .
...
#This function finds index of a city in array of "list_of_cities" .               
def findCity (c,city_list):
    for i in range(len(city_list)):
        if city_list[i].ID == c:
            return i
    return -1
#This function finds index of neighbor in all cities array of "list_of_cities" .  
def findNeighbor(n, neighbor):
    for i in range(len(neighbor)):
        ...
        if n_id == n :
          return i
    return -1
#This function removes visited city from neighbor of all cities .
def removeNeighbor(n,list_of_cities):
    for i in range(len(list_of_cities)):
        neighbor = list_of_cities[i].getNeighbors()
        j = findNeighbor(n,neighbor)
        if j != -1:  """if statement is added , if the index is not out of the range of our list ,
                          neighbor will be deleted """
         del neighbor[j]
# This function writes cost of minimum path and the minimum tour .   
...
cities = fileToList("test-input-3.txt")
total_cost,path = nearestNeighbor(cities)
print("Total cost :",total_cost)
listtoFile("output3.txt",total_cost,path)
