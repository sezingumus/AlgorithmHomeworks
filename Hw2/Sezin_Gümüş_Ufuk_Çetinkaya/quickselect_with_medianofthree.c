...
//Finding k th smallest number by using by QuickSelect and Median of three Algorithm
...
//It replaces the elements. It will be used in median3 method.
  ...
  printf("Enter your input file :\n");
   ...
   if (fptr == NULL) //If the entered input file does not exist.
    ...
    while (!feof (fptr)) //
     ...
     float sec = (float)(t2-t1)/CLOCKS_PER_SEC; //Finds the execution time of Quickselect with Median of Three Algorithm
